...
from app.core_bn.cfg_auth import get_current_cliente
...
def login(data: LoginClienteIn, db: Session = Depends(get_db)):
    ...
    if settings.DATA_BACKEND.lower() == "firebase":
        result = rep_firebase.login_cliente(data.numero_documento, data.password)
    else:
...
def perfil(db: Session = Depends(get_db), cli: dict = Depends(get_current_cliente)):
    if settings.DATA_BACKEND.lower() == "firebase":
        return rep_firebase.cliente_from_cuenta(cli["cliente_id"])
...
def cuentas(db: Session = Depends(get_db), cli: dict = Depends(get_current_cliente)):
    if settings.DATA_BACKEND.lower() == "firebase":
        return rep_firebase.cuentas_cliente(cli["cliente_id"])
...
def creditos(db: Session = Depends(get_db), cli: dict = Depends(get_current_cliente)):
    if settings.DATA_BACKEND.lower() == "firebase":
        return rep_firebase.creditos_cliente(cli["cliente_id"])
...
def movimientos(
    ...
    if settings.DATA_BACKEND.lower() == "firebase":
        return rep_firebase.movimientos_cliente(cli["cliente_id"], limit)
...
def notificaciones(db: Session = Depends(get_db), cli: dict = Depends(get_current_cliente)):
    if settings.DATA_BACKEND.lower() == "firebase":
        return rep_firebase.notificaciones_cliente(cli["cliente_id"])
...
def crear_operacion(
    ...
    if settings.DATA_BACKEND.lower() == "firebase":
        return rep_firebase.crear_operacion(cli["cliente_id"], data.model_dump())
